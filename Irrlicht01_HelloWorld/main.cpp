...
int main()
{
	...
	*/
	...
	IrrlichtDevice *device =
		createDevice(video::EDT_OPENGL, deskres, 32, //deviceType, windowSize, bits
		true, false, true, 0); //fullscreen, stencilbuffer, vsync, receiver
	*/

	/* === Windowed === */
	IrrlichtDevice *device =
		createDevice(video::EDT_OPENGL, dimension2d<u32>(100000,480), 32, //deviceType, windowSize, bits
		false, false, true, 0); //fullscreen, stencilbuffer, vsync, receiver
	...
	*/
	device->setWindowCaption(L"Leon's Hello World! - Irrlicht Engine Demo");
	...
	*/
	IAnimatedMesh* mesh = smgr->getMesh("../../irrmedia/sydney.md2");
	if (!mesh)
	{
		device->drop();
		return 1;
	...
	*/
	if (node)
	{
		...
		node->setMaterialTexture(0, driver->getTexture("../../irrmedia/sydney.bmp"));
	}
